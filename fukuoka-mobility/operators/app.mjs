...
async function json(path){const response=await fetch(path+'?v=20260915-1');if(!response.ok)throw Error(path+' を取得できません（'+response.status+'）');return response.json();}
...
function renderPriority(){
  $('#priority-operators').innerHTML=data.priorityGroups.map(group=>{const rs=selectOperators(data,{operator:group.id}),s=summarize(rs);return `<article class="priority-card"><span class="op-status available">公式時刻表の案内あり</span><h2>${esc(group.name)}</h2><b>${fmt(s.bus||s.stations)} <small>${s.bus?'停留所位置・2022年度':'駅グループ・2025年末'}</small></b><small>位置資料の集計。現在の全乗り場数ではありません。</small><a href="timetables.html?group=${group.id}"><strong>駅・停留所・路線の時刻表 →</strong></a><a href="?operator=${group.id}#operator-statistics">実績・収支と位置を見る →</a><small>全便の往復計算・リアルタイムは未接続</small></article>`;}).join('');
}
function render(){
...
function renderDetail(){
  ...
  $('#operator-detail').innerHTML=`<h2>${esc(group?.name??rs[0].name)}</h2>${rs.map(r=>`<p><strong>${esc(r.name)}</strong> ${badge(r)}<br>${esc(r.missingReason??r.timetable.note)}<br>${link(r.officialUrl,'公式案内')}${link(r.officialTimetableUrl,'公式の時刻表を確認')}${link(r.acquisitionSource,'取得元候補（未反映）')}</p>`).join('')}<p><strong>公式サイトへのリンクは、当サイトの分析へのデータ反映を意味しません。</strong></p><h3>出典と対象期間</h3>${rs.some(r=>r.busIndices.length)?`<p>${link(data.sources.bus.url,'国土数値情報・バス停留所')} / 2022年度。廃止・移設・会社変更を含む現況の確認は未完了。</p>`:''}${rs.some(r=>r.stationIndices.length)?`<p>${link(data.sources.rail.url,'国土数値情報・鉄道')} / 2025年12月31日。駅・線路位置のみ。時刻・停車便・運賃・遅延は未反映。</p>`:''}${fs.length?`<ul>${fs.map(f=>`<li>${esc(f.name)} — ${f.analysisSampleDates?'分析対象日 '+esc(f.analysisSampleDates.join('・')):esc(f.validFrom)+'〜'+esc(f.validTo)} /${f.validityNote?' '+esc(f.validityNote):''} ${f.dataStatus==='current'?'基準日の検証済み・収録分のみ':'期限外または検証エラー・計算に不使用'}<br>${link(f.sourceUrl,'公開元')}${link(f.licenseUrl,f.license)}</li>`).join('')}</ul>`:'<p>有効な時刻表ファイル：未反映。往復判定・便数・交通空白判定の計算対象に含みません。</p>'}<details><summary>原典の路線名 ${fmt(routeNames.length)}件を確認</summary><p>事業者内の表記を重複除去。現在運行中の路線数ではありません。位置資料とGTFSの名称は未統合です。</p><ul class="op-route-list">${routeNames.map(n=>`<li>${esc(n)}</li>`).join('')}</ul></details>`;
}
...
async function drawMap(){
  ...
  for(const r of rows){
    ...
    for(const i of r.busIndices){const f=bus.features[i],p=f.properties;dot(f.geometry.coordinates,p.P11_001,`${esc(r.name)}<br>停留所位置資料：2022年度<br><strong>この位置資料の運行本数・現況は未確認</strong><br>${esc(p.P11_003_01)}<br><a href="timetables.html?q=${encodeURIComponent(p.P11_001)}">この停留所の時刻表を探す →</a>`,'#a46b39');}
    const seen=new Set();for(const i of r.stationIndices){const f=rail.stations.features[i],p=f.properties;if(seen.has(p.N02_005g))continue;seen.add(p.N02_005g);dot(pointOf(f),p.N02_005,`${esc(r.name)} / ${esc(p.N02_003)}<br>駅位置：2025年12月31日<br><strong>全便の往復計算は未接続</strong><br><a href="timetables.html?q=${encodeURIComponent(p.N02_005)}">この駅の公式時刻表 →</a>`,'#2857a0');}
...
async function start(){
  ...
  $('#operator-select').innerHTML='<option value="all">すべての掲載主体</option><optgroup label="主要4事業者">'+data.priorityGroups.map(g=>`<option value="${g.id}">${esc(g.name)}</option>`).join('')+'</optgroup><optgroup label="事業者・公表主体">'+data.operators.map(r=>`<option value="${r.id}">${esc(r.name)}（${MODES[r.mode]}）</option>`).join('')+'</optgroup>';
