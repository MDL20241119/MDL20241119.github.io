...
const feeds=new Map();let cache=null,cacheKey='',referenceStops=null;
self.onmessage=async({data})=>{
  ...
  try{
    ...
    const k=profile.date+'|'+files.map(f=>f.file).sort().join('|');if(k!==cacheKey){cache=prepareGapNetwork(list,profile.date,referenceStops);self.postMessage({id,progress:'OSMの道路を準備しています',percent:0});cache.network.walkingGraph=await loadWalkingGraph();cacheKey=k;}
