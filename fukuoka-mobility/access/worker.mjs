...
self.onmessage=async({data})=>{
  ...
  try{
    ...
    const cache=date+'|'+files.map(f=>f.file).sort().join('|');if(cache!==networkKey){network=prepareAccess(list,date);self.postMessage({id,progress:'OSMの道路を準備しています'});network.walkingGraph=await loadWalkingGraph();networkKey=cache;}
    const dataEvidence={feeds:list.map(f=>({name:f.name,hash:f.hash,coverage:feedCoverage(f,date)})),excluded:network.excluded,walking:network.walkingGraph.meta};
