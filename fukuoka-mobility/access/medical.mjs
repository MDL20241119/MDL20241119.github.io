...
export async function renderMedical(container,id,file){
  ...
  try{
    ...
    container.innerHTML=`<details><summary>公表された曜日別の${f.category==='pharmacy'?'営業時間':'診療・受付時刻'}</summary><p>${esc(f.asOf)}時点の通常週。祝日・特定週・臨時休診・受入条件は施設へ確認してください。下の時刻は往復条件へ自動適用していません。</p>${f.schedules.length?f.schedules.map(s=>`<h3>${esc(s.department)}</h3><table><thead><tr><th>曜日</th><th>${f.category==='pharmacy'?'営業':'診療'}</th><th>外来受付</th></tr></thead><tbody>${Object.entries(s.days).flatMap(([day,slots])=>slots.map(slot=>`<tr><th>${esc(day)}</th><td>${esc(range(slot.care))}</td><td>${esc(range(slot.reception))}</td></tr>`)).join('')}</tbody></table>`).join(''):'<p>通常週の時刻は原典に記載がありません。</p>'}<p>${safeLink(f.hoursSourceResourceUrl,'時刻の原典データ')} · ${safeLink(f.sourceUrl,'厚生労働省の公開元')}</p></details>`;
