...
## 0.16.0：スマートフォンの操作を4つの画面に整理

700px以下では画面下の大きなメニューで切り替える。ユーザーは「地図・チャット・内容確認・依頼状況」、ドライバーは「受付・運行・地図・チャット」、管理者は「状況・地図・チャット・依頼一覧」。同じ入力欄・依頼データを表示し分けるため、切替だけで依頼を送信せず、入力途中の文や選択した場所・人数も保持する。広い画面に戻すと従来のマニュアル風グリッドを表示する。

スマホの本文と吹き出しは18〜21pxを基本とし、人数は3つの大きなボタン、主要操作は62〜68px、下部メニューは76px。地図とチャットの後は「次へ」から内容確認へ進める。依頼の保存後は状況画面、ドライバーの依頼選択後は運行画面、管理者の検索結果からは該当する記録へ移る。LINE風チャット、Neo Swissの色・罫線・数字、Coreの権限・停車確認・最終確認を維持する。

チャット入力中は下部メニューを退避し、送信後に戻す。セーフエリアと画面の高さに合わせるCSSを追加。地図を再表示すると表示範囲を再計算する。初期データやCoreには変更を加えていない。

従来の37項目に加え、スマホ用の入力保持・画面切替・横幅変更・依頼から操作への移動を実際のPyodide CoreとJSDOMで確認する。`YOKO_TEST_MOBILE=1` を付けて従来の2つのUI試験を実行すると、`mobile-journey-result.json` と `mobile-operations-result.json` を保存する。レイアウト・実機の仮想キーボードはJSDOMの検証範囲には含まない。

UIだけの再生成は `python3 scripts/yoko-demo/build.py --source /path/to/yoko-elevator --ui-only`。既存の `core-bundle.json` とデモ保存履歴を変えずにHTML/CSS/JSを再生成できる。

2026-09-20 公開確認：スマホの入力保持・役割切替などを含む49項目に合格。公開デモでも地図＋チャット入力、依頼保存、再読込、引受、車両確定表示、到着、乗車、降車、管理者の検索から記録への移動を確認した。320×667・390×844の表示領域では横はみ出しがなく、3役とも地図の3ピンが収まる。1280px幅では元の2列表示に戻り、入力途中の文も保持された。確認記録は `tests/yoko-demo/mobile-browser-result.json`。表示幅の検査はデスクトップChromiumのiframeで実施し、実機スマホ・Safari・実際のソフトウェアキーボードは未試験。
