...
## 0.17.0：マニュアルと同じ、地図からチャットへ続くスマホ体験

ユーザー画面はPCでも最大540pxのスマホ型に統一し、「地図が上、LINE風チャットが下」の一続きの画面に戻した。場所は地図でも文章でも選べ、人数は大きな3択で入力できる。地図とチャットを切り替えるメニューは使わない。下部の固定ボタンは、未入力の項目を案内し、選び終わると元のCore確認画面を開く。入力例は内容を埋めるだけで、依頼は確定しない。
