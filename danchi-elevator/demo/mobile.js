...
(function(){
  ...
  window.YokoMobile={create({getState}){
    ...
    let role=null,view=null,firstSnapshot=true,activeId=null,parts=[];
    const heading=document.createElement('div');heading.id='mobile-heading';heading.className='mobile-heading';heading.hidden=true;
    heading.innerHTML='<div><p id="mobile-section-label" class="mobile-section-label"></p><h2 id="mobile-view-title" tabindex="-1"></h2><p id="mobile-view-help"></p></div><button id="mobile-refresh" type="button" aria-label="最新の状況に更新">↻</button>';
    ...
    function leave(){clearParts();role=null;view=null;heading.hidden=true;nav.hidden=true;document.body.classList.remove('mobile-ui','mobile-input-focus');delete document.body.dataset.mobileView;}
    function sync(){
    ...
    function refreshSelection(){
      ...
      const button=$('mobile-continue'),s=getState(),hasRoute=$('origin').value&&$('destination').value&&$('origin').value!==$('destination').value;
      const active=s.snapshot?.rides.some(r=>!['completed','cancelled'].includes(r.status));
      button.disabled=!!s.busy||!!s.pending||(!active&&!hasRoute);button.textContent=active&&!s.editing?'いまの依頼を見る →':!hasRoute?'乗る場所と降りる場所を選択':$('passengers').value?'次へ：内容を確認する →':'次へ：人数を選ぶ →';
    }
    ...
    $('mobile-refresh').addEventListener('click',()=>$('refresh').click());
