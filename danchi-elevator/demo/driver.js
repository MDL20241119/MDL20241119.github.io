...
(function(){
  ...
  window.YokoDriver={create({getState,notify,perform,setHistory,onSelect=()=>false,preferredRide=()=>null}){
    ...
    const busy=()=>submitting||getState().busy||!!getState().pending||getState().user?.role!=='driver';
    ...
    function sync(){
      ...
      if(!selected())selectedId=live.find(r=>r.id===preferredRide())?.id||running[0]?.id||waiting[0]?.id||null;
    ...
    function leave(){active=false;selectedId=null;pendingAction=null;lastKey='';submitting=false;$('stopped').disabled=false;document.body.classList.remove('driver-ui');$('driver-panel').hidden=true;$('driver-action-dialog').close();$('driver-help-dialog').close();if($('rides-section').parentElement!==historyParent)historyParent.insertBefore($('rides-section'),historyNext);}
    ...
    $('driver-map-fit').addEventListener('click',map.fit);$('driver-map-return').addEventListener('click',()=>document.querySelector('.driver-board').scrollIntoView({block:'start',behavior:'auto'}));$('driver-next').addEventListener('click',()=>review());
