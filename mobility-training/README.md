...
大分は「35活動・750人以上」を、MDL活動集計として表示。750人以上は講義・協議・フィールドワーク・実証参加等を含む延べ参加人数であり、ユニーク人数ではありません。事業側の報告であることをUSER_REPORTEDとして保持し、独立した第三者検証とは区別します。異なる集計の公開noteを750の直接根拠にはしていません。

## 国の事業とMDLの実績

交通空白37紹介事例とモビリティ人材育成61採択事業を、異なる入口・検索範囲で表示します。MDL大分は後者に含まれる1事業として、写真・活動集計・実運行結果を専用セクションと実績ページで紹介します。追加の採択件数として加算しません。

事例名の主リンクは国交省の公式資料です。交通37件と人材育成の個別紹介3件は公式紹介ページ、残る58採択事業は国土交通省の正式採択一覧PDFへ接続します。後者はリンク名にPDFと明記します。MDLの分類・項目別出典は「MDLの整理・出典」から開きます。
