...
for(const m of boundary.features){for(const res of [500,1000]){const mesh=await read('gaps/data/'+m.properties.code+'-'+res+'.geojson');assert(mesh.features.length>0);assert.equal(new Set(mesh.features.map(f=>f.id)).size,mesh.features.length);}}
