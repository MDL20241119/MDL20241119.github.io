...
export const GAP_VERSION='oita-gaps-1.0';
...
export function defaultProfile(cityCode,files=[]){return {schema:1,cityCode,name:'検討用の初期条件',note:'',date:'2026-09-09',resolution:500,populatedOnly:true,combine:'any',distanceOn:true,distanceM:300,frequencyOn:true,frequencyRadius:500,minimumTrips:6,serviceStart:'06:00',serviceEnd:'22:00',activityOn:false,destinations:[],destinationRule:'any',departure:'09:00',deadline:'16:00',activityFrom:'10:00',activityTo:'11:00',dwell:50,legMinutes:90,maxWalk:500,totalWalk:2000,walkSpeed:4,walkFactor:1.3,transfers:1,activityScope:'view',files};}
export function activityOptions(p){return {departure:timeSeconds(p.departure),deadline:timeSeconds(p.deadline),activityFrom:timeSeconds(p.activityFrom),activityTo:timeSeconds(p.activityTo),dwell:p.dwell,legMinutes:p.legMinutes,maxWalk:p.maxWalk,totalWalk:p.totalWalk,walkSpeed:p.walkSpeed,walkFactor:p.walkFactor,maxTransfers:p.transfers,budget:null,wheelchair:false,reservation:'no'};}
export function validateProfile(p){
  if(!p||p.schema!==1||!/^44\d{3}$/.test(p.cityCode)||!validDate(p.date))throw Error('自治体・日付を確認してください');
  ...
  combineDeficits([p.distanceOn,p.frequencyOn,p.activityOn].filter(Boolean).map(()=>'unknown'),p.combine);
  ...
  if(!Array.isArray(p.files)||p.files.length<1||p.files.length>16||new Set(p.files).size!==p.files.length)throw Error('交通データを1つ以上選んでください');
