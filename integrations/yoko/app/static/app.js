...
const labels = {rider:'利用者',driver:'ドライバー',admin:'管理者'};
...
function pendingKey(){return `yoko-pending:${state.user.id}`;}
...
function signedIn(result){
  ...
  $('account').innerHTML=`<span>${e(labels[state.user.role])}<small>${e(state.user.id)}</small></span><button id="logout" class="text-button" type="button">ログアウト</button>`;
...
function render(){
  ...
  $('sync-state').textContent=`${date(snap.as_of)} 時点（日本時間）`;$('sync-state').classList.remove('disconnected');
}
