...
export default async function Page({params}:{params:Promise<{id:string}>}){const p=await browserPrincipal();const s=await readState(p.workspace) as State;try{const a=getProposal(s,p,(await params).id);return <Approval id={a.id} csrf={a.csrf} version={s.version} summary={a.summary} status={a.status} expiresAt={a.expiresAt} stale={a.baseVersion!==s.version}/>;}catch{return <main style={{padding:40,fontSize:20}}><h1>確認内容が見つかりません</h1><p>このアカウントで確認できる内容を開いてください。</p><a href="/connections">連携の画面へ戻る</a></main>;}}
